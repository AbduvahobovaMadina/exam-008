...
const Model: React.FC<any> = ({
  ...
  return (
    <>
      <div className="flex gap-2 items-center bg-inherit border-non cursor-pointer" onClick={showModal} >
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="link" href="https://google.com" type="primary" loading={loading} onClick={handleOk} > Search on Google</Button>,
        ]}
      >
          ...
          <div className="w-full md:w-1/2 text-white flex flex-col justify-between">
            <div>
              <div className="flex items-center space-x-3">
                <img className="w-10 h-10 rounded-full" src={owber} alt="User Profile" />
                <div>
                  <p className="text-lg font-semibold">{fullName}</p>
                  <p className="text-gray-400 text-sm">{date}</p>
                </div>
              </div>
              <p className="mt-4">{caption}{" "} <span className="text-blue-400">#nature #mountains</span> </p>
            </div>
              ...
              <div className="flex space-x-6">
                ...
                <button className="flex items-center space-x-2">
                  <svg className="w-5 h-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20" >
                    <path fillRule="evenodd" d="M18 10c0 3.866-3.582 7-8 7s-8-3.134-8-7 3.582-7 8-7 8 3.134 8 7zm-8-3a1 1 0 00-1 1v4a1 1 0 001 1h.007a1 1 0 001-1V8a1 1 0 00-1-1H10z" clipRule="evenodd" />
                  </svg>
                  <span>{}</span>
                </button>
                <button className="flex items-center space-x-2">
